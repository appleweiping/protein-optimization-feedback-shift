"""Closed-loop experiment runner shell for Day 2 data-environment work."""
...
class ClosedLoopRunner:
    """Minimal runner that validates the Day 2 data environment and records artifacts."""
    ...
    def run(self) -> dict[str, Any]:
        """Run the Day 2 data-environment validation flow."""
        ...
        split_result = build_split(
            dataset_bundle,
            self.config.dataset.to_dict(),
            processed_dir=project_root / "data" / "processed",
            logger=self.logger,
        )
        oracle = Oracle(dataset_bundle.records, logger=self.logger)
        oracle_validation = self._validate_oracle(split_result, oracle)
        ...
        trace_payload = {
            ...
            "stage": "day2_data_environment",
            "status": "ok",
            "timestamp": datetime.now().isoformat(timespec="seconds"),
            "message": "Data environment validated. Dataset loading, split construction, and oracle checks completed.",
        }
        ...
        summary = {
            "run_id": self.context["run_id"],
            "status": "completed",
            "message": "Day 2 data environment completed successfully.",
    ...
    def _validate_oracle(self, split_result: SplitResult, oracle: Oracle) -> dict[str, Any]:
        """Query a deterministic sample of candidate sequences and verify exact matches."""
        sample_count = min(
        ...
        queried = oracle.batch_query([record.sequence for record in sampled_records])
